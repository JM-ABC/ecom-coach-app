// ============================================
// Shell layout — sidebar + topbar + main panel
// Linear/Notion editor-tool feel
// ============================================

const { useState } = React;

// Inline SVG icons (stroke-based, Lucide-style)
const Icon = ({ name, size = 16, stroke = 1.8 }) => {
  const paths = {
    sparkles: 'M12 3L13.5 8.5L19 10L13.5 11.5L12 17L10.5 11.5L5 10L10.5 8.5L12 3Z M19 3L19.75 5L21.5 5.75L19.75 6.5L19 8.5L18.25 6.5L16.5 5.75L18.25 5L19 3Z',
    search: 'M11 19a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z M21 21l-4.3-4.3',
    pen: 'M12 20h9 M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z',
    layout: 'M3 3h18v18H3z M3 9h18 M9 21V9',
    calendar: 'M8 2v4 M16 2v4 M3 10h18 M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z',
    help: 'M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3 M12 17h.01',
    chevLeft: 'M15 18l-6-6 6-6',
    chevRight: 'M9 18l6-6-6-6',
    chevDown: 'M6 9l6 6 6-6',
    chevUp: 'M18 15l-6-6-6 6',
    settings: 'M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z M12 15.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z',
    crown: 'M2 4l3 12h14l3-12-6 7-4-7-4 7-6-7z M2 16h20',
    zap: 'M13 2L3 14h9l-1 8 10-12h-9l1-8z',
    target: 'M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z M12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12z M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4z',
    trendUp: 'M22 7l-8.5 8.5-5-5L2 17 M16 7h6v6',
    check: 'M20 6L9 17l-5-5',
    checkCircle: 'M22 11.08V12a10 10 0 1 1-5.93-9.14 M22 4L12 14.01l-3-3',
    circle: 'M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z',
    plus: 'M12 5v14 M5 12h14',
    x: 'M18 6L6 18 M6 6l12 12',
    filter: 'M22 3H2l8 9.46V19l4 2v-8.54L22 3z',
    tag: 'M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z M7 7h.01',
    alert: 'M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z M12 9v4 M12 17h.01',
    clock: 'M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z M12 6v6l4 2',
    list: 'M8 6h13 M8 12h13 M8 18h13 M3 6h.01 M3 12h.01 M3 18h.01',
    grid: 'M3 3h7v7H3z M14 3h7v7h-7z M14 14h7v7h-7z M3 14h7v7H3z',
    arrowRight: 'M5 12h14 M12 5l7 7-7 7',
    package: 'M16.5 9.4l-9-5.19 M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z M3.27 6.96L12 12.01l8.73-5.05 M12 22.08V12',
    flag: 'M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z M4 22V15',
    lightbulb: 'M9 18h6 M10 22h4 M15.09 14A7 7 0 1 0 8.91 14a4 4 0 0 1 1.09 2.74V18h4v-1.26A4 4 0 0 1 15.09 14z',
    trending: 'M23 6l-9.5 9.5-5-5L1 18 M17 6h6v6',
    share: 'M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8 M16 6l-4-4-4 4 M12 2v13',
    copy: 'M20 9h-9a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-9a2 2 0 0 0-2-2z M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1',
    download: 'M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4 M7 10l5 5 5-5 M12 15V3',
    refresh: 'M23 4v6h-6 M1 20v-6h6 M3.51 9a9 9 0 0 1 14.85-3.36L23 10 M1 14l4.64 4.36A9 9 0 0 0 20.49 15',
    users: 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M23 21v-2a4 4 0 0 0-3-3.87 M16 3.13a4 4 0 0 1 0 7.75',
    box: 'M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z',
    image: 'M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2z M8.5 10a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z M21 15l-5-5L5 21',
    sun: 'M12 2v2 M12 20v2 M4.93 4.93l1.41 1.41 M17.66 17.66l1.41 1.41 M2 12h2 M20 12h2 M6.34 17.66l-1.41 1.41 M19.07 4.93l-1.41 1.41 M12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12z',
    cloudRain: 'M16 13v8 M8 13v8 M12 15v8 M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25',
    eye: 'M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z',
  };
  const d = paths[name];
  if (!d) return null;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
         stroke="currentColor" strokeWidth={stroke} strokeLinecap="round" strokeLinejoin="round"
         style={{ flexShrink: 0, display: 'block' }}>
      {d.split(' M').map((p, i) => (
        <path key={i} d={i === 0 ? p : 'M' + p} />
      ))}
    </svg>
  );
};

const sidebarStyles = {
  sidebar: {
    width: 236,
    flexShrink: 0,
    background: 'var(--bg-subtle)',
    borderRightWidth: 1, borderRightStyle: 'solid', borderRightColor: 'var(--border)',
    display: 'flex',
    flexDirection: 'column',
    height: '100vh',
    position: 'sticky',
    top: 0,
    padding: '12px 10px',
  },
  brand: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    padding: '8px 8px 12px',
  },
  brandMark: {
    width: 24, height: 24,
    borderRadius: 7,
    background: 'linear-gradient(135deg, var(--accent) 0%, var(--accent-hover) 100%)',
    color: '#fff',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.18), 0 1px 2px rgba(17,24,39,0.08)',
  },
  brandName: {
    fontSize: 14,
    fontWeight: 700,
    letterSpacing: '-0.02em',
    color: 'var(--text)',
    fontFeatureSettings: "'ss02'",
  },
  group: {
    marginTop: 8,
  },
  groupLabel: {
    padding: '8px 8px 6px',
    fontSize: 11,
    fontWeight: 500,
    color: 'var(--text-subtle)',
    textTransform: 'uppercase',
    letterSpacing: '0.04em',
  },
  navItem: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    padding: '6px 8px',
    borderRadius: 6,
    fontSize: 13,
    fontWeight: 500,
    color: 'var(--text-muted)',
    cursor: 'pointer',
    marginBottom: 1,
    letterSpacing: '-0.005em',
    transition: 'background 120ms, color 120ms',
  },
  navItemActive: {
    background: 'var(--surface)',
    color: 'var(--text)',
    boxShadow: 'var(--shadow-sm)',
    borderWidth: 1, borderStyle: 'solid', borderColor: 'var(--border)',
  },
  spacer: { flex: 1 },
  footer: {
    borderTopWidth: 1, borderTopStyle: 'solid', borderTopColor: 'var(--border)',
    paddingTop: 10,
    marginTop: 8,
  },
  usageBar: {
    padding: '8px',
    borderRadius: 8,
    background: 'var(--surface)',
    borderWidth: 1, borderStyle: 'solid', borderColor: 'var(--border)',
    marginBottom: 8,
  },
  usageHead: {
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
    marginBottom: 6,
  },
  usageTitle: { fontSize: 11.5, fontWeight: 500, color: 'var(--text-muted)' },
  usageCount: { fontSize: 11.5, fontWeight: 600, color: 'var(--text)', fontVariantNumeric: 'tabular-nums' },
  usageTrack: {
    height: 4, borderRadius: 2,
    background: 'var(--bg-sunken)',
    overflow: 'hidden',
  },
  usageFill: {
    height: '100%',
    background: 'var(--accent)',
    borderRadius: 2,
  },
  userRow: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    padding: '6px 4px',
  },
  avatar: {
    width: 24, height: 24,
    borderRadius: 6,
    background: 'oklch(0.90 0.02 258)',
    color: 'var(--accent-text)',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    fontSize: 11, fontWeight: 600,
  },
  userName: { fontSize: 12.5, fontWeight: 500, color: 'var(--text)' },
  userMeta: { fontSize: 11, color: 'var(--text-subtle)' },
};

const Sidebar = ({ activeTab, onTabChange }) => {
  const tabs = [
    { id: 'calendar', label: '마케팅 캘린더', icon: 'calendar' },
    { id: 'product-name', label: '상품명 최적화', icon: 'search' },
    { id: 'copy', label: '카피 생성', icon: 'pen' },
    { id: 'detail-page', label: '상세페이지', icon: 'layout' },
  ];

  const tools = [
    { id: 'trends', label: '트렌드 리포트', icon: 'trending' },
    { id: 'competitor', label: '경쟁사 모니터링', icon: 'eye' },
  ];

  return (
    <aside style={sidebarStyles.sidebar}>
      <div style={sidebarStyles.brand}>
        <div style={sidebarStyles.brandMark}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block' }}>
            <path d="M4 7l8-4 8 4v10l-8 4-8-4V7z" />
            <path d="M4 7l8 4 8-4" />
            <path d="M12 11v10" />
          </svg>
        </div>
        <div style={sidebarStyles.brandName}>
          Seller<span style={{ color: 'var(--accent)' }}>kit</span>
        </div>
        <div style={{ flex: 1 }} />
        <button style={{ ...sidebarStyles.navItem, padding: 4, marginBottom: 0, color: 'var(--text-subtle)' }}>
          <Icon name="chevDown" size={14} />
        </button>
      </div>

      <div style={{ padding: '0 8px', marginBottom: 6 }}>
        <button className="btn sm" style={{ width: '100%', justifyContent: 'flex-start', color: 'var(--text-subtle)', gap: 6 }}>
          <Icon name="search" size={13} />
          <span>검색</span>
          <span style={{ flex: 1 }} />
          <span className="kbd">⌘K</span>
        </button>
      </div>

      <div style={sidebarStyles.group}>
        <div style={sidebarStyles.groupLabel}>도구</div>
        {tabs.map(t => (
          <div
            key={t.id}
            onClick={() => onTabChange(t.id)}
            style={{
              ...sidebarStyles.navItem,
              ...(activeTab === t.id ? sidebarStyles.navItemActive : {}),
            }}
          >
            <Icon name={t.icon} size={15} />
            <span>{t.label}</span>
          </div>
        ))}
      </div>

      <div style={sidebarStyles.group}>
        <div style={sidebarStyles.groupLabel}>인사이트</div>
        {tools.map(t => (
          <div key={t.id} style={sidebarStyles.navItem}>
            <Icon name={t.icon} size={15} />
            <span>{t.label}</span>
            <span style={{ flex: 1 }} />
            <span className="chip" style={{ fontSize: 9.5, padding: '0 5px', height: 16 }}>Soon</span>
          </div>
        ))}
      </div>

      <div style={sidebarStyles.spacer} />

      <div style={sidebarStyles.footer}>
        <div style={sidebarStyles.usageBar}>
          <div style={sidebarStyles.usageHead}>
            <span style={sidebarStyles.usageTitle}>오늘 사용량</span>
            <span style={sidebarStyles.usageCount}>7 / 10</span>
          </div>
          <div style={sidebarStyles.usageTrack}>
            <div style={{ ...sidebarStyles.usageFill, width: '70%' }} />
          </div>
          <button className="btn sm" style={{ width: '100%', marginTop: 8, background: 'var(--accent)', color: '#fff', borderColor: 'var(--accent)' }}>
            <Icon name="crown" size={11} />
            Pro 업그레이드
          </button>
        </div>
        <div style={sidebarStyles.userRow}>
          <div style={sidebarStyles.avatar}>박</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={sidebarStyles.userName}>박지민</div>
            <div style={sidebarStyles.userMeta}>쿠팡 · 생활용품</div>
          </div>
          <button className="btn icon sm ghost"><Icon name="settings" size={13} /></button>
        </div>
      </div>
    </aside>
  );
};

window.Sidebar = Sidebar;
window.Icon = Icon;
