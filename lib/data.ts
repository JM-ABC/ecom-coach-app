...
const PLATFORM_TIPS: Record<string, Partial<Record<EventType, PlatformInsight>>> = {
  ...
  momq: {
    holiday: { tip: '자사몰 CRM이 핵심 레버. 공휴일 10일 전 구매 이력 기반 세그먼트별 맞춤 쿠폰 발송. 공휴일 전날 "맘쑹배송 — 오늘 주문하면 내일 도착" 배너로 긴박감을 더하면 전환율이 올라갑니다.', action: '세그먼트 쿠폰 + 맘쑹배송 마감 배너', metric: '재구매 전환 유도' },
...
export const EVENTS: MarketingEvent[] = [
  ...
  },
  {
    ...
    checklist: [
      ...
      { d: -3, task: '맘쑹배송·로켓 재고 점검', done: false },
  ...
  },
  {
    ...
    products: [
      { name: '맘쑹배송 완구·교구 (내일 도착)', reason: '5/4 오후 급구매 대응 — 오늘 주문하면 내일 도착', urgency: 'high', category: 'b_toy', priceRange: { min: 15000, max: 70000 }, prepDays: 7, momqTip: '맘쑹배송 라벨 강조 (오늘 주문→내일 도착 보장). 맘큐 회원 전용 "긴급 쿠폰" D-1~D-0 한정 발행.' },
    ...
    checklist: [
      ...
      { d: -3, task: '맘쑹배송 가능 상품 별도 전시', done: false },
      { d: 0, task: '실시간 재고 모니터링', done: false },
    ],
    pro: '5월 4일 오후 3시~6시가 "급구매" 골든타임입니다. 맘쑹배송 가능 상품(주문 마감 전)만 상단 노출시키고, 재고 0인 상품은 숨기세요.',
  },
  {
    ...
    checklist: [
      ...
      { d: 0, task: '맘쑹배송 주문 마감 전 긴급 푸시알림', done: false },
    ],
