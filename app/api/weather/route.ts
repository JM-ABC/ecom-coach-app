...
function getBaseDateTime(): { date: string; time: string } {
  ...
  // 유효 발표시각 (각 시각 +10분 이후 사용 가능)
  const bases = [2, 5, 8, 11, 14, 17, 20, 23];
  let selectedBase = 23;
  for (const b of bases) {
    if (total >= b * 60 + 10) selectedBase = b;
  }

  const baseDate = new Date(kst);
  // 자정~02:09 구간은 전날 2300 기준
  if (selectedBase === 23 && hour < 2) {
  ...
  return {
    date: `${y}${m}${d}`,
    time: String(selectedBase).padStart(2, '0') + '00',
  };
}
...
function generateWeatherEvents(items: KmaItem[]): MarketingEvent[] {
  const byDate: Record<string, DayData> = {};
  for (const item of items) {
    if (!byDate[item.fcstDate]) byDate[item.fcstDate] = { temps: [], pty: [], wsd: [] };
    const v = parseFloat(item.fcstValue);
    if (item.category === 'TMP') byDate[item.fcstDate].temps.push(v);
    if (item.category === 'PTY') byDate[item.fcstDate].pty.push(v);
    if (item.category === 'WSD') byDate[item.fcstDate].wsd.push(v);
  }
  ...
  const conds: DayCond[] = dates.map(d => ({
    ...
    hasRain: byDate[d].pty.some(v => v === 1 || v === 4),
    hasSnow: byDate[d].pty.some(v => v === 2 || v === 3),
  }));
...
export async function GET() {
  ...
  try {
    ...
    // 공공데이터포털 키는 이미 URL인코딩된 형태(%XX)로 발급됨 → 그대로 URL에 삽입
    // 디코딩된 키(+,/ 포함)를 저장한 경우에도 encodeURIComponent로 처리
    const isAlreadyEncoded = /%[0-9A-Fa-f]{2}/.test(apiKey);
    const encodedKey = isAlreadyEncoded ? apiKey : encodeURIComponent(apiKey);
    const qs = `ServiceKey=${encodedKey}&pageNo=1&numOfRows=1000&dataType=JSON&base_date=${date}&base_time=${time}&nx=${NX}&ny=${NY}`;

    const res = await fetch(`${KMA_URL}?${qs}`, {
      next: { revalidate: 3600 },
    });

    if (!res.ok) {
      const body = await res.text();
      throw new Error(`KMA HTTP ${res.status} — ${body.slice(0, 200)}`);
    }

    const json = await res.json();
    const resultCode = json?.response?.header?.resultCode;
    if (resultCode !== '00') {
      throw new Error(`KMA result code: ${resultCode} - ${json?.response?.header?.resultMsg}`);
    }
