...
유료 가입제. 4등급(라이트/스탠다드/레귤러/스페셜) 구성이며 "프리미엄 멤버십"은 프로그램 전체 명칭입니다. 상위 등급일수록 가입축하 포인트·매월 쇼핑지원금·전용 특가 혜택이 커집니다. 혜택 설계 시 이 등급 체계를 활용하세요.

| 등급 | 가입비 | 이용기간 | 최대 혜택 규모 |
|------|--------|----------|----------------|
| 라이트 | 6,000원 | 100일 | 최대 17,000원 |
| 스탠다드 | 20,000원 | 365일 | 최대 70,000원 |
| 레귤러 | 50,000원 | 365일 | 최대 147,500원 |
| 스페셜 | 500,000원 | 365일 | 최대 1,438,000원 |

공통 혜택 (등급별 차등 — 상위 등급일수록 규모 확대):
- 가입축하 포인트 (맘큐 포인트는 현금처럼 사용 가능)
- 매월 쇼핑지원금
- 프리미엄 멤버십 전용 특가·기획전
- 등급별 추가 할인

가입 부가 혜택:
- 얼리버드: 맘큐 신규가입 후 7일 이내 멤버십 가입 시 7,000원 추가 혜택
- 추천인: 추천인 링크 가입 시 총 7,000P (기본 가입축하 2,000P + 추천인 5,000P)
- 앱전용 30% 쿠폰: 구매 시 발급, 14일 유효, 최대 20,000원 할인
- 멤버십 재가입 보너스

주의: 등급별 정확한 쇼핑지원금 금액·추가 할인율은 시기별로 변동되며 위 표 외 구체 수치는 확정 데이터가 아닙니다. 단정하지 말고 "등급이 올라갈수록 혜택이 커지는" 차등 구조로 설계하고, 추정 수치를 쓸 경우 반드시 "추정치"로 표기하세요.
...
### 체험단 & 커뮤니티 후기 전략
### 멤버십 등급별 혜택 설계
(라이트/스탠다드/레귤러/스페셜 등급별 차등 쿠폰·포인트·단독가 설계. 비회원→라이트→스탠다드→레귤러→스페셜 업그레이드 유도 시나리오 포함)
### 라이브방송 / 콘텐츠 마케팅 계획
### MD 단독 기획전 구성
